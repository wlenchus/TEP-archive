...
PREDICTIONS (stated before run; same-session, unhashed — disclosed as such):
  ...
  E-b  For exactly one s in {+1,-1} (tau(chi) vs tau(chibar)) the ratio
       ...
       eps\*sqrt(N)/tau\_s lands on a 20th root of unity to \<= 3e-9, where
  ...
  E-c  (deferred to exact\_epsilon\_confirm.py) the identified algebraic candidate
  ...
  E-d  The SAME structural formula (same s; unit expressed through the field's own
       ...
       a\_N as unit \= zeta20^k0 \* a\_N^e for some fixed e in {0,+1,-1}), applied

       verbatim at 2141 with its own CSV-pinned chi and a\_N \= zeta10^8, reproduces
...
Run 1 also refuted the mu20 guess for local roots: |a\_p| \= 1 classes (group-element
...
ANEXP \= {}    \# a\_N \= zeta10^m, read from CSV ramified row
...
def vec\_from\_counts(n):  \# sum n\_m zeta^m \-\> basis vector
...
for N in (1951, 2141):
    ...
    with open(CSV\[N\]) as f:
        ...
        for row in csv.DictReader(f):
            ...
            if row\['j5'\].strip() \== '-':
                ...
                s \= row\['a\_p\_exact'\].strip(); assert s.startswith('zeta10^')
    ...
    print(f"     chi(-1) \= zeta5^{jarr\[N-1\]} (must be 0: even character); a\_N \= zeta10^{ANEXP\[N\]}")
    ...
    for (a, b) in \[(1,1), (1,2), (1,3)\]:
        ...
        J \= vec\_from\_counts(n); Js.append(J)
...
ok\_Eb \= best\[0\] \<= 3e-9 and second\[0\] \>= 1e6\*best\[0\]
...
print(f"E-b (identification clause \+ amended separation, second/best \= {second\[0\]/best\[0\]:.1e}): "
      ...
      f"{'PASS' if ok\_Eb else 'FAIL/KILL'}")
...
if ok\_Eb:
    ...
    m1 \= ANEXP\[1951\]           \# a\_N \= zeta10^m1 \= zeta20^(2 m1)

    print(f"\\nunit structure at 1951: unit \= zeta20^{k1}; a\_N \= zeta20^{2\*m1}")

    for e in (0, 1, \-1):

        k0 \= (k1 \- e\*2\*m1) % 20

        print(f"   e={e:+d}:  unit \= zeta20^{k0} \* a\_N^{e}")

    \# cleanest direct form: const \= \-eps ?= tau/(sqrt(N)\*a\_N)  (Atkin-Li shape, no sign)

    dAL \= float(fabs(CONST\[1951\] \- tau/(sqrt(N)\*exp(mpc(0,1)\*pi\*m1/5))))

    print(f"   direct check: |const\_meas \- tau(chi)/(sqrt(N)\*a\_N)| \= {dAL:.3e}")
    ...
    for e in (0, 1, \-1):
        ...
        d \= float(fabs(pred \- eps2\_meas))

        verdicts.append((d, e, k0, complex(pred)))

        print(f"   e={e:+d} (unit \= zeta20^{k0}\*a\_N^{e}):  predicted eps \= {complex(pred):.9f}   "

              f"|pred \- eps\_meas| \= {d:.3e}   {'PASS' if d \<= 1e-8 else 'no'}")
...
\# \---------- Theorem-A support: local roots are in mu20 (per (a\_p, chi\_p) class)
...
def parse\_apex(sst):
    ...
    if sst.endswith("\*1/phi"): fac \= 1/phi\_f; sst \= sst\[:-6\]

    elif sst.endswith("\*phi"): fac \= phi\_f; sst \= sst\[:-4\]
...
with open(CSV\[1951\]) as f:
    ...
    for row in csv.DictReader(f):
        ...
        classes.add((row\['a\_p\_exact'\].strip(), row\['chi\_p\_exact'\].strip()))

worst\_abs \= 0.0; worst\_mu60 \= 0.0; orders \= set()
...
for (astr, cstr) in sorted(classes):
    ...
    for r in ((a+disc)/2, (a-disc)/2):
        ...
        worst\_abs \= max(worst\_abs, float(fabs(abs(r)-1)))

        worst\_mu60 \= max(worst\_mu60, float(fabs(r\*\*60 \- 1)))
...
print(f"{len(classes)} distinct (a\_p, chi\_p) classes; worst ||root|-1| \= {worst\_abs:.2e}; "

      f"worst |root^60 \- 1| \= {worst\_mu60:.2e}")

print(f"exact multiplicative orders observed: {sorted(orders)}  (all divide 60)")

print("=\> every local Frobenius root is a 60th root of unity; |c\_n| \<= d(n) follows")