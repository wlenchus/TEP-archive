...
1. **[EXECUTED → refined] Dead-channel theorem** — the rung layer is built (Thread Q); remaining: formalize supply = #{new (form, ±, χ) directions with L(f⊗χ, 1) ≠ 0}; run the rank-2 hostage (N = 389); meet Booker–Krishnamurthy exact statements; lift from the weight-2 abelianized shadow to the nonabelian generation problem.
