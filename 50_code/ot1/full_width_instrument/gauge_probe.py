"""Unsketched solve of chi=-4 emb=0; null-space support diagnosis + gauge search."""
...
print(f"UNSKETCHED system {A.shape}",flush=True)
t0=time.time()
...
for c in range(cols):
    ...
    piv.append(c); r+=1
    if r==rows: break
print(f"unsketched aug rank={len(piv)}, inconsistent={nT in piv} ({time.time()-t0:.0f}s)",flush=True)
if nT not in piv:
    ...
    for j,v in enumerate(null):
        supp=[Ts[k] for k in np.nonzero(v)[0][:12]]
        supp_str=[('.'.join(map(str,sorted(t))) ) for t in supp]
        print(f"null[{j}] weight={int((v!=0).sum())} sample support: {supp_str[:8]}")
    cls={q:pd[q][0] for q in pd}
    sing_idx={k:next(iter(Ts[k])) for k in range(nT) if len(Ts[k])==1}
    free_sing=[sing_idx[k] for k in free if k in sing_idx]
    print("free singleton primes:",sorted(free_sing)," classes:",[cls.get(q) for q in sorted(free_sing)])
    import itertools, json
    pm=lambda x:x==1 or x==P-1
    found=[]
    for combo in itertools.product([1,P-1],repeat=len(free)):
        v=sol.copy()
        # force free coords to +-1 via null basis (free coords are unit vectors in null basis)
        for j,fc in enumerate(free):
            delta=np.mod(combo[j]-v[fc],P)
        ...
        if ok: found.append(combo)
    print(f"GAUGE SEARCH: {len(found)} all-pm multiplicative solutions out of {2**len(free)} combos")
    print("VERDICT:", "PASS-candidate — decode and pre-register" if found else "REJECT-BY-GAUGE at this config")
