...
Kill: if no solution with |n_i| <= 3 exists over {C10, C6, C4, C2}, report
...
def cos_pi_t_over_5(t):
    t %= 10
    return C5T[t if t <= 5 else 10 - t]
def cos_2pi_jk_over(m, jk):    # cos(2*pi*jk/m) for m in {10,6,4,2}
    if m == 10: return cos_pi_t_over_5(jk % 10)
    if m == 6:
...
for col in COLS:
    ...
    for chi in FAITH:
        s = ZERO
        for k in range(m):
            s = add(s, mul(cos_2pi_jk_over(m, j*k), T[chi][CLI[fus[k]]]))
        s = (s[0]/m, s[1]/m)
...
for col in COLS:
    (h, m), j = col
    dim = G//m
    print(f"Ind_{h}(l^{j})".rjust(14) + f"  {dim:3d}  "
          + "  ".join(f"{M[(col,chi)]:3d}" for chi in FAITH))

# ---------- solve: sum n_col * M[col] = e_{2a} over the faithful irreps
...
print(f"\ninteger solutions with |n_i| <= 3: {len(sols)}; minimal-L1 solution:")
best = sols[0]
terms = [(ni, col) for ni, col in zip(best, COLS) if ni != 0]
for ni, ((h, m), j) in terms:
    print(f"   {ni:+d} * Ind_{h}(lambda^{j})   [dim {G//m}]")
...
def ind_value(col, c):
    ...
    s = ZERO
    for k in hits:
        s = add(s, cos_2pi_jk_over(m, j*k))
    return smul(Fr(G, m*SZ[c]), s)
for c in range(9):
    tot = ZERO
    for ni, col in zip(best, COLS):
        if ni: tot = add(tot, smul(Fr(ni), ind_value(col, c)))
    assert tot == T['2a'][c], (CL[c], tot, T['2a'][c])
print("verified: the combination equals theta2 = chi_2a EXACTLY on all 9 classes")
...
n_by_dim = [(ni, 120//m, h, j) for ni, ((h, m), j) in zip(best, COLS) if ni]
num = [f"L_{d}(s; {h}, l^{j})^{ni}" for ni, d, h, j in n_by_dim if ni > 0]
den = [f"L_{d}(s; {h}, l^{j})^{-ni}" for ni, d, h, j in n_by_dim if ni < 0]
