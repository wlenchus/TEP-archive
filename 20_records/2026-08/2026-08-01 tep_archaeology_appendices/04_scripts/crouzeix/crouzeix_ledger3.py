...
reactive), u_eff = 1 - ||Tx||^2/4 (total deficit: dilation leak + spectral detuning).
EXACT constraint to verify: (c^2+I)^2 + J^2 <= 4 c^2 (1 - u_eff)   [circle ledger]
Prediction: at extremals (c->2): I,J,u_eff all -> 0 (total channel collapse into
...
def ledger(A, coef, sig, Ms):
    ...
    coef = coef/sup; f_b = f_b/sup           # normalize sup|f|=1
...
def run(name, A, deg=5, restarts=15):
    ...
    for _ in range(restarts):
        r = minimize(negc, rng.standard_normal(2*(deg+1)), method='Nelder-Mead',
    ...
    print(f"{name:26s} c={c:7.4f}  I={Iv:+9.5f}  J(rot)={Jv:+9.5f}  u_eff(abs)={ueff:+8.5f}  circle:{ok} (lhs={lhs:8.4f} rhs={rhs:8.4f})")

run("J2=[[0,2],[0,0]] extremal", np.array([[0,2],[0,0]], complex))
run("J3 nilpotent extremal",     np.array([[0,1,0],[0,0,1],[0,0,0]], complex))
A = rng.standard_normal((3,3)) + 1j*rng.standard_normal((3,3)); run("random 3x3", A)
