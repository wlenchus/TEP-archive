...
The PARI half of the diagonal probe:
  (1) build L(s, Ad rho~) from the certified Galois-side table (unramified p: local poly (1-X)(1 - t_p X + X^2),
      t_p = |a_p|^2 - 2 ; p = 1951: (1 - X) ; conductor 1951^2 ; Gamma_R(s)^3 ; root number to be determined),
  (2) check its functional equation with PARI (lfuncheckfeq) for eps = +1 and eps = -1,
  (3) compute L(1, Ad) and the predicted Rankin-Selberg constant c = L(1,Ad)/zeta(2) * 1951/1952,
...
mp.dps = 30
N = 1951
path = '/home/claude/deposit_supplement_20260830/data/hecke_eigenvalues_doud1951_to1e5.csv'
...
"""
open('/tmp/ad.gp','w').write(gp_script)
subprocess.Popen('nohup gp -q -f /tmp/ad.gp > /tmp/ad_gp.log 2>&1 &', shell=True)
print('gp launched in background -> /tmp/ad_gp.log')
